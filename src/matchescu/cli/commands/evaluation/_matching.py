...
def _compute_metrics(true_matches: set, graph: ReferenceGraph, direction: str = "fwd") -> dict:
    actual = set(graph.matches(min_weight=0.0)) # ensures we don't filter out any matches
    return {
        f"precision_{direction}": precision(true_matches, actual),
        f"recall_{direction}": recall(true_matches, actual),
        f"f1_{direction}": f1(true_matches, actual),
    }
...
@click.pass_context
def main(ctx: click.Context, output_dir: str | os.PathLike, stats_csv_path: str | os.PathLike):
    ...
    output_path = make_absolute_path(output_dir, root_dir)
    ...
    with Progress() as progress:
        for ds_config in cfg.benchmark_data:
            ...
            cs_true_matches = set(cmp for cmp in cs if cmp in benchmark_data.true_matches)
            ...
            for model_config in cfg.models:
                ...
                fwd_graph = ReferenceGraph(matcher, directed=True)
                rev_graph = ReferenceGraph(matcher, directed=True)
                for x, y in cs_refs:
                    fwd_graph.add(x, y)
                    rev_graph.add(y, x)
                    progress.update(model_task, advance=1)
                    progress.update(ds_task, advance=1)
                fwd_persistence = GmlGraphPersistence(
                    output_path / f"{model_config.name}-{benchmark_data.name}-fwd.gml"
                )
                fwd_graph.save(fwd_persistence)
                rev_persistence = GmlGraphPersistence(
                    output_path / f"{model_config.name}-{benchmark_data.name}-rev.gml"
                )
                rev_graph.save(rev_persistence)
                stats.append({
                    "dataset": benchmark_data.name,
                    "model": model_config.name,
                    **_compute_metrics(cs_true_matches, fwd_graph, direction="fwd"),
                    **_compute_metrics(cs_true_matches, rev_graph, direction="rev"),
                })
